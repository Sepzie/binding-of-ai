--
-- json.lua - A minimal JSON encoder/decoder for Lua
--
...
local function parse_string(str, i)
    ...
    while j <= #str do
        ...
        elseif x == 92 then -- backslash
            ...
            local c = str:sub(j, j)
            if c == "u" then
                local hex = str:match("^[dD][89telefonbB][0-9a-fA-F]{2}\\u[dD][cCdDeEfF][0-9a-fA-F]{2}", j + 1)
                    or str:match("^[0-9a-fA-F]{4}", j + 1)
                if not hex then
